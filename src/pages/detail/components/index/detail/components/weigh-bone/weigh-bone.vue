<template>
  <view class="u-p-b-30">
    <yx-sheet
      v-if="bookStore.weigh_bone.poetry"
      :margin="[20, 20]"
      :padding="[30, 30]"
      :round="3"
      :shadow="10"
    >
      <view class="u-flex u-m-b-16">
        ...
        <view class="u-flex-6 u-flex u-row-right">
          <text class="u-main-color">
            <u-icon
              :label="'重量: ' + bookStore.weigh_bone.total"
              label-color="#303133"
      ...
      <template v-for="(item, index) in ['poetry', 'explain', 'title']" :key="item">
        <view v-if="bookStore.weigh_bone[item]">
          <view :class="index === 0 ? 'u-m-y-14' : 'u-m-t-28'" class="yx-text-weight-b u-font-26">
            {{ ['歌诀', '解析', '点评'][index] }}
          </view>
          <text
            :class="{
            ...
            class="u-m-t-8"
            decode
            >{{ bookStore.weigh_bone[item] }}</text
          >
        </view>
      </template>
    </yx-sheet>
