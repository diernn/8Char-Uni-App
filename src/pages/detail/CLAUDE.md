...
- `components/index/basic/components/table/table.vue`：基础命盘主表，按固定 10 行配置把 `detailStore.start/top/bottom/bottom_hide/trend/selfsit/nayin/empty/gods` 组装成二维表；`showTips()` 只对白名单字段触发 `yx-book-tips`，其中 `main/assiste` 被映射到 `relation` 类型，`selfsit` 被映射到 `trend` 类型。该表强依赖 `detailStore` 的多组嵌套对象 shape 稳定。
...
- 当前高风险区域：
  ...
  - `table.vue` 依赖多组嵌套对象 shape，并同时承接 tips 事件分发；若后端字段结构漂移，最容易出现局部空列或错误弹层
...
- `src/pages/detail/components/index/basic/components/table/table.vue`
