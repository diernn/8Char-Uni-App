<template>
  <u-popup closeable v-model="show" mode="bottom">
    <template v-for="type in ['top', '', 'bottom']" :key="type || 'middle'">
      <scroll-view
      ...
      >
        <view v-for="(item, index) in props[type].mark" :key="`${type}-${item.label}-${index}`">
          <view
          ...
          <view class="u-flex u-rela u-p-b-50">
            ...
            ></view>
            <view
              v-for="(mitem, mindex) in item.mark"
              :key="`${type}-${item.label}-${mitem.label}-${mindex}`"
              class="u-abso pillar-round"
              :class="[`u-type-${getElAttr(mitem.label)}-bg`]"
              :style="{
      ...
      <view v-else class="u-flex u-text-center">
        <view v-for="(item, index) in content" :key="`middle-${index}`">
          <view
            v-for="(label, labelIndex) in item"
            :key="`middle-${index}-${label}-${labelIndex}`"
...
const content = computed(() => {
  return [TITLE_LIST.slice(0, colNum.value), props.top.list, props.bottom.list];
});
